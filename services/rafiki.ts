...
export interface QuoteRequest {
  ...
  paymentMethod: 'bank_account' | 'mobile_wallet' | 'cash_pickup';
}
...
class RafikiService {
   ...
   */
  public async getQuote(data: QuoteRequest): Promise<QuoteResponse> {
    if (!this.isAvailable) {
   ...
   */
  public async initiateTransfer(data: TransferRequest): Promise<TransferResponse> {
    if (!this.isAvailable) {
   ...
   * @returns Promise<TransferResponse>
   */
  public async checkTransferStatus(transferId: string): Promise<TransferResponse> {
    if (!this.isAvailable) {
      return {
        id: transferId,
        status: 'failed',
   ...
   */
  private mockQuoteResponse(data: QuoteRequest): QuoteResponse {
   ...
   */
  private mockTransferResponse(data: TransferRequest, status: TransactionStatus): TransferResponse {
    const reference = data.reference || uuidv4();
    const exchangeRate = this.getMockExchangeRate(data.sourceCurrency, data.destinationCurrency);
   ...
   */
  private getMockExchangeRate(sourceCurrency: string, destinationCurrency: string): number {
