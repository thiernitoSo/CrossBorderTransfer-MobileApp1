...
class OrangeMoneyService {
   ...
   */
  public async initiatePayment(data: TransactionRequest): Promise<TransactionResponse> {
    if (!this.isAvailable) {
   ...
   * @returns Promise<TransactionResponse>
   */
  public async checkTransactionStatus(transactionId: string): Promise<TransactionResponse> {
    if (!this.isAvailable) {
      return {
        id: transactionId,
        status: 'failed',
   ...
   */
  private mockTransactionResponse(data: TransactionRequest, status: TransactionStatus): TransactionResponse {
    const reference = data.externalReference || uuidv4();
